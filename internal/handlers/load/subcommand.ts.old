...
export default async (client: CustomClient, cmd: SubCommand): Promise<void> => {
    return await new Promise<void>(async (resolve) => {
        const basePath = path.join(__dirname, '..', '..', 'interactions', 'commands', cmd.data.category as string, cmd.command.name);
        for (const subCommand of fs.readdirSync(basePath).filter(file => file.endsWith('.ts') || file.endsWith('.js'))) {
            const subCmd: SubCommandRun = (await import(path.join(basePath, subCommand))).default;
            if (!('run' in subCmd)) {
